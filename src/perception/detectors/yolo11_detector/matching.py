...
def score_reacquisition_match(reference_features, candidate, frame, predicted_center=None):
    ...
    # 1. Shape Matching (Hu Moments + Contour) - 30 points
    ...
    # 2. Color Distribution (3D Histogram) - 25 points
    ...
    # 3. Spatial Color Layout (Region histograms) - 20 points
    layout_scores = []
    ...
    # 6. Position (Low weight) - 5 points
    if predicted_center is not None:
        frame_diagonal = math.hypot(frame.shape[1], frame.shape[0])
